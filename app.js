...
client.on('message', message => 
{
		...
		switch(cmd)
		{
			...
			case markets:
				...
				var req_sp500_val = "https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=^DJI&interval=1min&apikey=" + stockApiKey;
				var req_nasdaq_val = "https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=^DJI&interval=1min&apikey=" + stockApiKey;
				var req_rus2000_val = "https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=^DJI&interval=1min&apikey=" + stockApiKey;
				...
				request(req_sp500_val, function(error, response, body) 
				{
					const content = JSON.parse(body);
					var lastRef = content['Meta Data']['3. Last Refreshed'];
					sp500 = content['Time Series (1min)'][lastRef]['3. close'];
				});
				...
				request(req_nasdaq_val, function(error, response, body) 
				{
					const content = JSON.parse(body);
					var lastRef = content['Meta Data']['3. Last Refreshed'];
					nasdaq = content['Time Series (1min)'][lastRef]['3. close'];
				});
				...
				var ax = post(":chart_with_downwards_trend: __**Market Indices**__ :chart_with_upwards_trend:\nDow Jones Industrial Average (**^DJI**): **" + dji + "**");
				if(dji_dir == 1)
				{
					ax += "<:_bull_:602373998214512670>+" + dji_change + "%";
				} else {
					ax += "<:_bear_:602374398959288321>-" + dji_change + "%";	
				}
				
				ax += "\nS&P 500 (**^INX**): **" + sp500 + "**";
				if(sp500_dir == 1)
				{
					ax += "<:_bull_:602373998214512670>+" + sp500_change + "%";
				} else {
					ax += "<:_bear_:602374398959288321>-" + sp500_change + "%";	
				}
				
				ax += "\nNASDAQ Composite (**^IXIC**): **" + nasdaq + "**";
				if(sp500_dir == 1)
				{
					ax += "<:_bull_:602373998214512670>+" + nasdaq_change + "%";
				} else {
					ax += "<:_bear_:602374398959288321>-" + nasdaq_change + "%";	
				}
				
				ax += "\nRussell 2000 (**^IXIC**): **" + rus2000 + "**";
				if(rus2000_dir == 1)
				{
					ax += "<:_bull_:602373998214512670>+" + rus2000_change + "%";
				} else {
					ax += "<:_bear_:602374398959288321>-" + rus2000_change + "%";	
				}
				
				ax += "\n*For additional information regarding individual stocks, do /stock <ticker>*";
